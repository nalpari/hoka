...
java -jar target/bo-0.0.1-SNAPSHOT.jar    # 빌드한 jar로 실행
```

기본 포트는 `8080`입니다. hoka-fo-api도 기본 포트가 `8080`이라 둘을 함께 띄우면 충돌합니다. 이때는 이 API를 `8081`로 띄우고, 호출 예시의 포트도 `8081`로 바꿉니다.

```bash
./mvnw spring-boot:run -Dspring-boot.run.arguments=--server.port=8081
java -jar target/bo-0.0.1-SNAPSHOT.jar --server.port=8081
```
